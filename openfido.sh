...
if [ "$ANALYSIS" = "vegetation_analysis" ]; then 
    ...
    if [[ "$USECASE" = "all" ]]; then
        for option in "${USECASES[@]}"; do
            gridlabd pole_analysis $OPENFIDO_OUTPUT/$GLM_NAME.glm --analysis=$option --output=$OPENFIDO_OUTPUT/results_$option.csv $POLE_OPTION
            # --poles_selected=POLENAME
        done
    else
        gridlabd pole_analysis $OPENFIDO_OUTPUT/$GLM_NAME.glm --analysis=$USECASE --output=$OPENFIDO_OUTPUT/results_$USECASE.csv $POLE_OPTION
        # --poles_selected=POLENAME
    fi
fi 
