...
if [ $USECASE = "BULK" ]; then
    ...
    gridlabd --verbose -D output_message_context=NONE -D starttime=$STARTTIME -D stoptime=$STOPTIME -D timezone=$TIMEZONE -D WIND_SPEED="$OPENFIDO_INPUT/$WIND_SPEED" main_bulk.glm $OPENFIDO_OUTPUT/POLES.glm 
