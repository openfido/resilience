...
if [ $USECASE = "INCLUDE_NETWORK" ]; then
    ...
    gridlabd create_childs -i=$OPENFIDO_OUTPUT/${MODEL_NAME}.json -o=.$OPENFIDO_OUTPUT/${MODEL_NAME}_childs.glm -P='class:node,name:^ND_' -C='class:load,nominal_voltage:{nominal_voltage},phases:{phases},constant_power_B:0+0jkVA'
